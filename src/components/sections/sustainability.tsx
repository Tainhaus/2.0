        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-10">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {GREEN_PILLARS.map((pillar) => (
              <div key={pillar.title} className="flex gap-3 p-4 rounded-2xl bg-white/5 border border-white/10">
                ...
                <div>
                  <h3 className="font-display text-sm font-bold text-white mb-1">{pillar.title}</h3>
                  <p className="font-body text-xs text-forest-200 leading-relaxed">{pillar.description}</p>
                </div>
              </div>
            ))}
          </div>
